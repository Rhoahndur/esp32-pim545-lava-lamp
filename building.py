#!/usr/bin/env python3
"""9x17 RGB lava lamp for the Green Building simulator.

Same blob physics as the PIM545 firmware (7x17 white). This client POSTs
color frames to the HTTP sim. Pass --controller so corner taps on the pack
drop matching pebbles here.

Python 3.10+; pyserial only if you use --controller.
"""
...
MAX_FAILURES = 10
HTTP_TIMEOUT = 10
USER_AGENT = "esp32-pim545-lava-lamp/building"
PROGRESS_EVERY = 2.0
...
class Ripples:
    ...
    def drop(self, x: float, y: float, amp: float = 1.0) -> None:
        # New ring on every tap. Age-out in step(); only evict the oldest
        # if we somehow exceed the cap.
        self.drops.append((x, y, 0.0, amp))
        if len(self.drops) > WAVE_MAX_DROPS:
            self.drops.sort(key=lambda d: d[2])
            self.drops = self.drops[:WAVE_MAX_DROPS]
